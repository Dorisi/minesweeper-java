...
public class Frame extends JFrame {
	...
	private void jbInit() throws Exception {
		...
		start.addActionListener(new Frame_start_actionAdapter(this));
	...
	public void start_actionPerformed(ActionEvent e) {
		...
		if (num >= 5 && num < 50) {
			...
			JOptionPane.showMessageDialog(null, "您设置的地雷数太少了,请重设!", "错误", JOptionPane.ERROR_MESSAGE);
			num = 10;
			BombNum = num;
		} else {
				...
				if (bombButton[i][j].isBomb != true) {
					for (int x = i - 1; x < i + 2; x++) {
						for (int y = j - 1; y < j + 2; y++) {
							if ((x >= 0) && (y >= 0) && (x < ((int) Math.sqrt(BlockNum)))
		...
		for (int x = i - 1; x < i + 2; x++) {
			for (int y = j - 1; y < j + 2; y++) {
				if (((x != i) || (y != j)) && (x >= 0) && (y >= 0) && (x < ((int) Math.sqrt(BlockNum)))
						&& (y < ((int) Math.sqrt(BlockNum)))) {
					if (bombButton[x][y].isBomb == false && bombButton[x][y].isClicked == false
